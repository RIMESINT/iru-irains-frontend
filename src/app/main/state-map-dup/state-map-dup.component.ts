...
//         const id2 = feature.properties["state_code"];
//         const matchedData = this.findMatchingData(id2);
//         const rainfall = matchedData?.departure != null ? matchedData.departure : "NA";
//         const color = this.constants.getColorForRainfall(rainfall);
...
//       },
//       onEachFeature: (feature: any, layer: any) => {
//         this.bindPopover(feature, layer, true);
//       },
//     }).addTo(this.map);
//   });
// }
// private bindPopover(feature: any, layer: any, isState: boolean): void {
//   const id = isState ? feature.properties["state_code"] : feature.properties["district_c"];
//   const name = isState ? feature.properties["state_name"] : feature.properties["district"];
//   const matchedData = this.findMatchingData(id);
...
//   const normalRainfall = matchedData?.normal_rainfall != null
//     ? `${this.constants.trimToOneDecimals(parseFloat(matchedData.normal_rainfall))} mm`
//     : "NA";

//   const popupContent = `
//     <div style="background-color: white; padding: 5px; font-family: Arial, sans-serif;">
//       <div style="color: #002467; font-weight: bold; font-size: 13px;">NAME: ${name}</div>
...
const stateDetails: Record<string, StateDetail> = {
    "ARUNACHAL PRADESH": { name: "ARUNACHAL PRADESH", lat: 29, lng: 94.2 },
    "LADAKH (UT)": { name: "LADAKH (UT)", lat: 35.3, lng: 76 },
    "JAMMU & KASHMIR (UT)": { name: "JK", lat: 33.5, lng: 75 },
    "HIMACHAL PRADESH": { name: "HP", lat: 32.7, lng: 76 },
    "PUNJAB": { name: "PUNJAB", lat: 31.5, lng: 73.8 },
    "CHANDIGARH (UT)": { name: "CH", lat: 30.8, lng: 76 },
    "UTTARAKHAND": { name: "UK", lat: 30.2, lng: 78.5 },
    "HARYANA": { name: "HR", lat: 29.5, lng: 75 },
    "DELHI (UT)": { name: "DL", lat: 28.7, lng: 77.1 },
    "UTTAR PRADESH": { name: "UP", lat: 27.2, lng: 79.2 },
    "RAJASTHAN": { name: "RAJASTHAN", lat: 27, lng: 72 },
    "GUJARAT": { name: "GUJARAT", lat: 23.5, lng: 71 },
    "MADHYA PRADESH": { name: "MADHYA PRADESH", lat: 23.9, lng: 76.5 },
    "DADRA & NAGAR HAVELI AND DAMAN & DIU (UT)": { name: "D & NH-D & D (UT)", lat: 21.5, lng: 72 },
    "MAHARASHTRA": { name: "MAHARASHTRA", lat: 19.5, lng: 74 },
    "TELANGANA": { name: "TELANGANA", lat: 18, lng: 77.7 },
    "GOA": { name: "GOA", lat: 16.5, lng: 72.7 },
    "KARNATAKA": { name: "KARNATAKA", lat: 15, lng: 74.7 },
    "ANDHRA PRADESH": { name: "ANDHRA PRADESH", lat: 15.5, lng: 78 },
    "TAMILNADU": { name: "TN", lat: 11.5, lng: 77.5 },
    "KERALA": { name: "KL", lat: 10.5, lng: 75.5 },
    "WEST BENGAL": { name: "WB", lat: 23.7, lng: 86.8 },
    "BIHAR": { name: "BIHAR", lat: 26, lng: 85.3 },
    "ASSAM": { name: "AS", lat: 26.8, lng: 91.9 },
    "MANIPUR": { name: "MP", lat: 24.5, lng: 92.3 },
    "ANDAMAN & NICOBAR ISLANDS (UT)": { name: "A & N Islands (UT)", lat: 9.8, lng: 91.7 },
    "LAKSHADWEEP (UT)" : {name: "LAKSH UT", lat: 10.8, lng: 72.8}
  ...
  this.http.get("assets/geojson/INDIA_STATE.json").subscribe((res: any) => {
    const stateLayer = L.geoJSON(res, {
      ...
      onEachFeature: (feature: any, layer: any) => {
        ...
        const stateData = stateDetails[stateName];

        let labelName = stateName;
        let center = { lat: feature.properties["lat"], lng: feature.properties["lng"] };

        // Override state label based on custom logic
        if (stateData) {
          labelName = isFullScreen ? stateName : stateData.name;
          center = { lat: stateData.lat, lng: stateData.lng };
        }
          ...
          const dailyRainfall = matchedData?.actual_state_rainfall != null
            ? `${this.constants.trimToOneDecimals(matchedData.actual_state_rainfall)} mm`
            : "NA";

          const normalRainfall = matchedData?.rainfall_normal_value != null
            ? `${this.constants.trimToOneDecimals(parseFloat(matchedData.rainfall_normal_value))} mm`
            : "NA";


        // Create label markers with customized position and content
        if (center.lat && center.lng) {
          const labelId = `label-${stateName}`;
      ...
      //         className: "state-label",
      //         html: `
      //            <div id="${labelId}" style="text-align: center; color: #002467; font-size: ${this.defaultFontSizeonMap}px; font-weight: bold;">
      //   <div>${labelName}</div>
      //   <div style="font-size: 13px;"> ${dailyRainfall}</div>
      //   <div style="font-size: 13px;"> ${normalRainfall}</div>
      //   <div style="font-size: 13px;"> ${rainfall} %</div>
      // </div>
      //         `,
      //         iconSize: isFullScreen ? [50, 10] : [80, 10],
      //       }),
      //     }).addTo(this.map);
      ...
      <div id="${labelId}" style="text-align: center; color: #002467; font-size: ${this.defaultFontSizeonMap}px; font-weight: bold;">
       ...
       <div>${labelName}</div>
        ...
        this.bindPopover(feature, layer, true, labelName, center);
      },
    }).addTo(this.map);
  });
}
...
private bindPopover(
  ...
  const name = labelName || (isState ? feature.properties["state_name"] : feature.properties["district"]);
  const matchedData = this.findMatchingData(id);
console.log('matchedData', matchedData);
console.log('matchedData', matchedData?.departure);
console.log('actual_state_rainfall', matchedData?.actual_state_rainfall);
console.log('rainfall_normal_value', typeof matchedData?.rainfall_normal_value);
  const rainfall = matchedData?.departure != null
    ? this.constants.trimToZeroDecimals(matchedData?.departure)
    : "NA";
  ...
  const normalRainfall = matchedData?.rainfall_normal_value != null
    ? `${this.constants.trimToOneDecimals(parseFloat(matchedData?.rainfall_normal_value))} mm`
    : "NA";
    console.log('data', rainfall, dailyRainfall, normalRainfall);
  ...
  // Optional: Use the `center` coordinates for map markers or labels if needed
  if (center) {
    console.log("Center for label:", center);
  }
}
