...
export class VerificationPageMcComponent {
  ...
  async fetchDataFromBackend(): Promise<void> {
    try {
      ...
      this.existingstationdata = value.filter((x: any) => x.rmc_mc.toUpperCase() === this.currentMCorRMC.toUpperCase());
      this.filterByDate();
    } catch (err) {
  ...
  filterByDate() {
    ...
    this.noOfNotVerfied = this.existingstationdata.filter((x: any) => x[columnToCheck] == 'null' || x[columnToCheck] == null).length;
    this.noOfVerified = this.existingstationdata.filter((x: any) => x[columnToCheck] != 'null' && x[columnToCheck] != null).length;
    this.noOfUpdateVerified = this.existingstationdata.filter((x: any) => x[curr_date] != -999.9).length;
    this.noOfUpdateNotVerified = this.existingstationdata.filter((x: any) => x[curr_date] == -999.9).length;
  }
  ...
  async Verifyall() {
    if(this.dataToDisplay.length > 0){
      if (confirm("Do you want to verify these stations?") === true) {
        const filterStationIds = this.dataToDisplay.map((x: any) => ({ 'station_id': Number(x.stationid) }));
        const data = {
